...
public class CommonUtil {
    ...
    public static void checkBaseParam(Entity entity) {
        if (entity == null) {
            throw new VoiceException(ErrorConstant.PARAMS_ILLEGE);
        }
        if (StringUtils.isBlank(entity.getChannel())) {
            throw new VoiceException(ErrorConstant.PARAMS_CHANNEL);
        }
        if (StringUtils.isBlank(entity.getProduct())) {
            throw new VoiceException(ErrorConstant.PARAMS_PRODUCT);
        }
        if (StringUtils.isBlank(entity.getVersion())) {
            throw new VoiceException(ErrorConstant.PARAMS_VERISON);
        }
        if (StringUtils.isBlank(entity.getIp())) {
            throw new VoiceException(ErrorConstant.PARAMS_REQUESTIP);
        }
    }
     ...
     * 验证电话号码 匹配格式：11位手机号码
     */
    public static void checkPhone(String str) throws VoiceException {
        if (StringUtils.isEmpty(str)) {
            logger.error("手机号为空");
            throw new VoiceException(ErrorConstant.PARAMS_PHONE);
        }
        String regex = "^(1(20|45|47)\\d{8})|(1(3|5|8)\\d{9})|(17\\d{9})$";
        if (!match(regex, str)) {
            logger.error("手机号格式错误",str);
            throw new VoiceException(ErrorConstant.PHONE_INVILID);
        }
    }
     ...
     */
    public static void checkTel(String str) throws VoiceException {
        if (StringUtils.isEmpty(str)) {
            throw new VoiceException(ErrorConstant.PARAMS_TEL);
        }
        String regex = "^\\d{8}$";
        if (!match(regex, str)) {
            throw new VoiceException(ErrorConstant.TEL_INVILID);
        }
    }
     ...
     * 校验密码
     */
    public static void checkPassword(String pwd) throws VoiceException {
        if (pwd == null || pwd.length() != 32) {
            logger.error("密码为空或者格式错误");
            throw new VoiceException(ErrorConstant.WRONG_PASSWORD);
        }
    }
    ...
    public static void checkRest(String restUrl, Map<String, String> params){
        if(StringUtils.isBlank(restUrl)){
            throw new VoiceException(ErrorConstant.ERROR_RESTURL_FAILED);
        }
    ...
    public static ResponseEntity doPost(String restUrl, Map<String, String> params, Map<String, String> header, ErrorConstant error){
        ...
        try {
            ...
            throw new VoiceException(error, e);
        }
    }
